...
const WearableGallery = ({ wearableMedia }: { wearableMedia: string[] }) => {
  const [mainImage, setMainImage] = useState(wearableMedia[0]);
  ...
  return (
    <div className="flex flex-col gap-3 w-1280">
      ...
      open={index >= 0}
      close={() => setIndex(-1)}
      slides={wearableMedia.map((image) => ({ src: image }))}
...
export default WearableGallery;
