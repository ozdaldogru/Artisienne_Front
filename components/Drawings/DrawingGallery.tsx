...
const DrawingGallery = ({ drawingMedia }: { drawingMedia: string[] }) => {
  const [mainImage, setMainImage] = useState(drawingMedia[0]);
  ...
  return (
    <div className="flex flex-col gap-3 w-1280">
      ...
      open={index >= 0}
      close={() => setIndex(-1)}
      slides={drawingMedia.map((image) => ({ src: image }))}
