...
const CrochetGallery = ({ crochetMedia }: { crochetMedia: string[] }) => {
  const [mainImage, setMainImage] = useState(crochetMedia[0]);
  ...
  return (
    <div className="flex flex-col gap-3 w-1280">
      ...
      open={index >= 0}
      close={() => setIndex(-1)}
      slides={crochetMedia.map((image) => ({ src: image }))}
