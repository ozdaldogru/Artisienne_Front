...
const PaintingGallery = ({ paintingMedia }: { paintingMedia: string[] }) => {
  const [mainImage, setMainImage] = useState(paintingMedia[0]);
  ...
  return (
    <div className="flex flex-col gap-3 w-1280">
      ...
      open={index >= 0}
      close={() => setIndex(-1)}
      slides={paintingMedia.map((image) => ({ src: image }))}
