...
const WoodBurningGallery = ({ woodburningMedia }: { woodburningMedia: string[] }) => {
  const [mainImage, setMainImage] = useState(woodburningMedia[0]);
  ...
  return (
    <div className="flex flex-col gap-3 w-1280">
      ...
      open={index >= 0}
      close={() => setIndex(-1)}
      slides={woodburningMedia.map((image) => ({ src: image }))}
    ...
    {woodburningMedia.map((image, index) => (
          <Image
            key={index}
            src={image}
            height={40}
...
export default WoodBurningGallery;
