...
const TattooGallery = ({ tattooMedia }: { tattooMedia: string[] }) => {
  const [mainImage, setMainImage] = useState(tattooMedia[0]);
  ...
  return (
    <div className="flex flex-col gap-3 w-1280">
      ...
      open={index >= 0}
      close={() => setIndex(-1)}
      slides={tattooMedia.map((image) => ({ src: image }))}
