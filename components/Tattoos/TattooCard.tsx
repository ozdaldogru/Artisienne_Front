  ...
  <Link
    ...
    >
            <div
              ...
              style={{ backgroundImage: `url(${tattoo.media[0]})` }}
            >
              ...
              <span className="absolute bottom-0 left-0 w-full bg-black bg-opacity-50 text-white text-center text-[30px] max-[431px]:text-[20px] rounded-md">
                      {tattoo.status === "Pending" ? (
                        ...
                        <p className="text-orange-500 text-bold " >{tattoo.status}</p>
               
                      ) : (
                      
                        tattoo.status === "Archived"  ? (
                        
                            <p className="text-blue-500 text-bold" >{tattoo.status}</p>
                       
                  ) : (
                      
                    tattoo.status === "Sold Out"  ? (
                      
                          <p className="text-red-500 text-bold" >{tattoo.status}</p>
                    
                ) :
                (
                     <p className="text-green-500 text-bold" >{tattoo.status}</p>
                 )))
                }
              </span>
      ...
      <div className="flex flex-col  w-full border-b border-[#8F95A0]">
        ...
        <p className="text-body-bold text-[#8F95A0] max-[431px]:text-[15px]">${tattoo.price.toLocaleString()}</p>
